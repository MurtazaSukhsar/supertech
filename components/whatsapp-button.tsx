...
export function WhatsAppButton() {
  ...
  // Hide while the chatbot window is open so it never covers the send button
  useEffect(() => {
    const handler = (e: Event) => {
      setChatbotOpen(Boolean((e as CustomEvent<{ open: boolean }>).detail?.open))
    }
  ...
  return (
    <a
      href={`${contactInfo.whatsappHref}?text=${encodeURIComponent(t.a11y.whatsappFloatMessage)}`}
      target="_blank"
